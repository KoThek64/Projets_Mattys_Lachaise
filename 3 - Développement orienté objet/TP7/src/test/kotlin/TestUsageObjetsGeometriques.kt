...
// TODO : renommer le fichier .ktest en .kt
    ...
    fun `dessiner un texte`() {
        val objet: Manipulable = Texte(4, 2, "La réponse universelle est 42")
        assertEquals("\"La réponse universelle est 42\":X=4,Y=2,P=Times,S=10", objet.dessiner())
    }

    @Test
    fun `deplacer, puis dessiner un rectangle`() {
        val objet: ObjetManipulable = Rectangle(4, 2, 420, 42)
        objet.deplacer(66,6)
    ...
    fun `deplacer, puis dessiner un texte`() {
        val objet: ObjetManipulable = Texte(4, 2, "La réponse universelle est 42")
        objet.deplacer(66,6)
        assertEquals("\"La réponse universelle est 42\":X=66,Y=6,P=Times,S=10", objet.dessiner())
    }

    @Test
    fun `regrouper des objets graphiques`() {
        val groupe = Groupe(Ellipse(4, 2, 420, 42),
