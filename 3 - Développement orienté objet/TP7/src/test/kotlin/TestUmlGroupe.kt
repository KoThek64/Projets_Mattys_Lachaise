...
class TestUmlGroupe {
	...
	fun `test3 - 'Groupe' has a constructor`() {
		uml.constructorCheck(paramsAndTypes=arrayOf(Pair("objet1", ObjetGraphique::class), Pair("objet2", ObjetGraphique::class)))
	}
	...
	fun `test6 - 'Groupe' has an attribute 'ObjetGraphiqueobjetsRegroupes'`() {
		uml.attributeCheck("objetsRegroupes", Array<ObjetGraphique?>::class)
	}
	...
	fun `test8 - 'Groupe' has a method 'regrouper'`() {
		uml.methodCheck("regrouper", Boolean::class, methParamAndTypes=arrayOf(Pair("objetAjoute", ObjetGraphique::class)))
	}
