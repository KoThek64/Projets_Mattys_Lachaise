...
class TestUmlPersonne {
	...
	fun `test3 - 'Personne' has a constructor`() {
		uml.constructorCheck(paramsAndTypes=arrayOf(Pair("n", String::class), Pair("p", String::class)))
	}
	...
	@Test
	fun `test6 - 'Personne' has an attribute 'nbOeuvresAchetees'`() {
		uml.attributeCheck("nbOeuvresAchetees", Int::class, KVisibility.PUBLIC)
	}

	@Test
	fun `test7 - 'Personne' has an attribute 'collection'`() {
		uml.attributeCheck("collection", Array<Oeuvre?>::class)
	}

	@Test
	fun `test8 - 'Personne' has method(s)`() {
		uml.methodNumber(1)
	}

	@Test
	fun `test9 - 'Personne' has a method 'acheter'`() {
		uml.methodCheck("achete", methParamAndTypes=arrayOf(Pair("nouvelleOeuvre", Oeuvre::class)))
	}
